/*
 * A large trade company has millions of products, each described by barcode,
...
namespace DataStructuresEfficiency
{
    ...
    public class TradeCompanyTest
    {
        ...
        public static void Main()
        {
            ...
            Console.WriteLine($"\rAdding products -> Elapsed time: {sw.Elapsed}");

            sw.Restart();
            SearchProductsInPriceRange(store); // 5 000 000 price searches
            sw.Stop();

            Console.WriteLine($"\nSearching products -> Elapsed time: {sw.Elapsed}\n");
        }
